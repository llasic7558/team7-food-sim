...
Setup
  ...
  docker compose logs --tail=50 -f order-dispatch-worker

  Host terminal 2:

  docker compose logs --tail=50 -f notification-worker

  Host terminal 3:

  docker compose exec holmes bash

  Inside the Holmes shell:

  # Optional baseline checks
  curl -s http://order-dispatch-worker:8110/health | jq '{status,queue_depth,retry_queue_depth,dlq_depth,last_job_at}'
  curl -s http://notification-worker:8000/health | jq '{status,queue_depth,dlq_depth,last_job_at}'

  Dispatch worker poison pill:

  redis-cli -h redis LLEN queue:order_dispatch:dlq
  redis-cli -h redis RPUSH queue:order_dispatch '{"broken_field":null}'
  redis-cli -h redis LLEN queue:order_dispatch:dlq
  curl -s http://order-dispatch-worker:8110/health | jq '{status,queue_depth,retry_queue_depth,dlq_depth,last_job_at}'

  Reset a known good order for the “still processes normally” demo:

  # Run these from a host terminal, not inside Holmes
  docker compose exec order-db psql -U app -d orders -c "UPDATE orders SET status='pending', driver_id=NULL, updated_at=NOW() WHERE id=3;"
  docker compose exec driver-db psql -U app -d drivers -c "DELETE FROM driver_assignments WHERE order_id='3'; UPDATE drivers SET status='Free', distance_from_order=NULL;"

  Good dispatch message after poison pill:

  # Back inside Holmes
  redis-cli -h redis RPUSH queue:order_dispatch '{"order_id":"3","restaurant_id":"3"}'
  curl -s http://order-service:8000/orders/3 | jq '{id,status,driver_id,restaurant_id}'
  curl -s http://order-dispatch-worker:8110/health | jq '{status,queue_depth,retry_queue_depth,dlq_depth,last_job_at}'

  Notification worker poison pill:

  redis-cli -h redis LLEN queue:notifications:dlq
  redis-cli -h redis RPUSH queue:notifications '{"order_id":null}'
  redis-cli -h redis LLEN queue:notifications:dlq
  curl -s http://notification-worker:8000/health | jq '{status,queue_depth,dlq_depth,last_job_at}'

  Good notification after poison pill:

  redis-cli -h redis RPUSH queue:notifications '{"order_id":"demo-003","event":"order_delivered","status":"delivered"}'
  curl -s http://notification-worker:8000/health | jq '{status,queue_depth,dlq_depth,last_job_at}'

  Step 4: Failure Scenarios

  Scenario 1: dispatch worker bad restaurant goes to DLQ

  # Inside Holmes
  redis-cli -h redis RPUSH queue:order_dispatch '{"order_id":"demo-bad-rest","restaurant_id":"999999"}'
  redis-cli -h redis LLEN queue:order_dispatch:dlq
  curl -s http://order-dispatch-worker:8110/health | jq '{status,queue_depth,retry_queue_depth,dlq_depth,last_job_at}'

  Scenario 2: no drivers available retries instead of orphaning the order

  # Host terminal: force all drivers busy and reset order 3
  docker compose exec order-db psql -U app -d orders -c "UPDATE orders SET status='pending', driver_id=NULL, updated_at=NOW() WHERE id=3;"
  docker compose exec driver-db psql -U app -d drivers -c "DELETE FROM driver_assignments WHERE order_id='3'; UPDATE drivers SET status='Busy', distance_from_order=NULL;"
  docker compose exec driver-db psql -U app -d drivers -c "SELECT id,name,status FROM drivers ORDER BY id;"

  # Inside Holmes: queue the job
  redis-cli -h redis RPUSH queue:order_dispatch '{"order_id":"3","restaurant_id":"3"}'
  redis-cli -h redis ZCARD queue:order_dispatch:retry
  curl -s http://order-service:8000/orders/3 | jq '{id,status,driver_id}'
  curl -s http://order-dispatch-worker:8110/health | jq '{status,queue_depth,retry_queue_depth,dlq_depth,last_job_at}'

  # Host terminal: recover drivers so a later retry can succeed
  docker compose exec driver-db psql -U app -d drivers -c "UPDATE drivers SET status='Free';"
  sleep 6
  docker compose exec driver-db psql -U app -d drivers -c "SELECT order_id,driver_id,final_status,completed_at FROM driver_assignments WHERE order_id='3';"

  # Inside Holmes: confirm the order was not left invisibly assigned
  curl -s http://order-service:8000/orders/3 | jq '{id,status,driver_id}'

  Scenario 3: duplicate notification is tolerated
  ...
  Host terminal for the duplicate-notification logs:

  docker compose logs --since=30s notification-worker

  Step 5: k6 Resilience Test

  Host terminal 4:

  docker compose exec holmes bash

  Inside that Holmes shell:
  ...
  Inside the second Holmes shell:
  ...
  Optional repo-specific notification load test:

  # Inside Holmes
  k6 run /workspace/k6/sprint-3-notification-poison.js

  # In another Holmes shell
  while true; do
    date
    curl -s http://notification-worker:8000/health | jq '{status,queue_depth,dlq_depth,last_job_at}'
    sleep 2
